...
const medium = (m, cat) => (m === "other" ? (cat ? cat.replace(/s$/, "") : "Artwork") : m.charAt(0).toUpperCase() + m.slice(1));
const listJoin = (a) => (a.length < 3 ? a.join(" and ") : `${a.slice(0, -1).join(", ")} and ${a[a.length - 1]}`);

const glyph = (w) => (w.m === "other" && /attraction/i.test(w.c || "") ? "spark" : GLYPH[w.m] || "brush");
function render(data, x, w) {
  ...
  const media = im
    ...
    : `<div class="wd-media photo halftone" data-prog="${esc(w.p)}"><span class="photo-missing">${I(glyph(w))}<span class="label">No photo yet</span></span></div>`;
  ...
  const facts = [
    ...
    w.c && w.c.toLowerCase() !== `${w.m}s` && w.m !== "other" ? ["Listed under", esc(w.c)] : null,
    (w.ak || []).length ? ["Also listed as", esc(w.ak.map((a) => `“${a}”`).join(", "))] : null,
  ].filter(Boolean);
  // BLINK's official map number (2026-09-24), with a link to the map itself when the program publishes one
  const pm = x.programs && x.programs[w.p];
  const mapNo = w.mn != null ? `<p class="wd-mapno" data-prog="${esc(w.p)}"><span class="mapno">${esc(`${(pm && pm.s) || prog.n} map No. ${w.mn}`)}</span>${pm && pm.map ? ext(pm.map[1], esc(pm.map[0])) : ""}</p>` : "";
  const inPlan = has(w.id);
  bodyEl.dataset.wd = w.id;
  bodyEl.innerHTML = `${media}
<p class="evd-kind label">${I(glyph(w))}${esc(medium(w.m, w.c))}</p>
<h2 id="wd-title" tabindex="-1">${esc(w.t)}</h2>
${w.at ? `<p class="wd-by">${esc(w.at)}</p>` : people.length ? `<p class="wd-by">${esc(listJoin(people.map(([, p]) => p.n)))}</p>` : `<p class="wd-by unk">${w.m === "other" ? "Credit not listed" : "Artist not listed"}</p>`}
${mapNo}
...
<p class="source-line">${I("info")}<span>Source${(w.as || []).length ? "s" : ""}: ${[w.src, ...(w.as || [])].map((u) => ext(u, esc(hostOf(u) || u))).join(" · ")}</span></p>
